...
2. If the user gives a cruise itinerary or multi-stop itinerary, treat each stop as part of a larger journey and suggest 1 to 3 strong-fit experiences per stop.
...
- For cruise or multi-stop itineraries, 1 to 3 experiences per stop is enough.
...
def call_model(user_prompt: str) -> dict:
    """
    Calls the OpenAI Responses API and returns parsed JSON.
    """
...
def render_experience_cards(experiences: list[dict]) -> None:
    """
    Render each recommendation as an expandable card.
    """
...
def render_multi_stop(stops: list[dict]) -> None:
    """
    Render multi-stop / cruise results.
    """
    for stop in stops:
        stop_name = stop.get("stop_name", "Stop")
        why_this_stop_matters = stop.get("why_this_stop_matters", "")
        experiences = stop.get("experiences", [])
...
with st.form("moment_finder_form"):
    ...
    if trip_mode == "Single destination":
        ...
        itinerary_text = st.text_area(
            ...
            placeholder="""March 31 - Amsterdam
April 1 - Amsterdam
April 2 - Brussels
April 3 - Paris
April 4 - At sea
